...
- [ ] **Resource tile inspector with role description + Demolish action.**
  - Tapping a resource tile (timber, stone, grain, clay) currently opens a confirm dialog asking "Clear the X resource?" — too transactional.
  - Instead, open a building-inspector-style panel: title (the resource name), 1–2 short rows describing the resource's role in the game (what extractor consumes it, what it processes into, etc.), and at the bottom a "Demolish" action that does what the confirm did (calls `clear_resource_tile`).
  - Same panel chrome and minimize/close affordances as the building inspector — visual + interaction parity.
